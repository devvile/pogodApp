...
/**
 * Displays weather icon and condition
 */
...
/**
 * Generic table cell wrapper with center alignment
 */
...
/**
 * Displays a metric value with optional comparison indicator
 */
...
/**
 * Displays city name with country and optional base city indicator
 */
export const CityNameCell = ({
  cityName,
  country,
  isBaseCity = false,
}: CityNameCellProps) => {
  return (
    <div className="flex items-center gap-3">
      <MapPin
        className={`${
          isBaseCity ? "text-blue-300" : "text-gray-400"
      ...
      <div>
        <div
          className={`font-medium ${
            isBaseCity ? "text-blue-200" : "text-white"
          } group-hover:text-blue-200`}
        >
          {cityName}
          {isBaseCity && (
            <span className="text-xs ml-2 text-blue-300">(base)</span>
          )}
        </div>
...
/**
 * Displays relative difference with color-coded arrows
 */
export const RelativeDifferenceCell = ({
  ...
  const difference = showDecimals
    ? relative.difference.toFixed(1)
    : relative.difference;
